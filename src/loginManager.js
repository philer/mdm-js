...
function find(fn, arr) {
  for (let i = 0, len = arr.length ; i < len ; ++i) if (fn(arr[i])) return arr[i];
}
...
export function getUser(user) {
  user = user.id || user;
  return find(usr => usr.id === user, users);
}
...
export function getSession(session) {
  ...
  return find(sess => sess.id === session, sessions);
}
...
export function getLanguage(language) {
  ...
  return find(lang => lang.id === language, languages);
}
